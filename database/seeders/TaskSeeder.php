...
class TaskSeeder extends Seeder
{
  ...
  public function run(): void
  {
    ...
    $this->command->info('Seeding Tasks and Assignments...');
    ...
    $this->command->info('Creating tasks and assigning children...');
    ...
    $this->command->info('Generating future task assignments (next 5 days)...');
    ...
    $this->command->info('Generating specific past/status assignments...');
    ...
    if ($numberOfChildren > 0) {
      ...
      $completedTaskDef = Task::where('title', 'Morning Exercise')->first(); // Needs_approval = false
      ...
      $pendingTaskDef = Task::where('title', 'Write Short Story')->first(); // Needs_approval = true
      ...
      // --- Scenario: Approved ---
      $approvedTaskDef = Task::where('title', 'Music Practice')->first(); // Needs_approval = true
      ...
      $rejectedTaskDef = Task::where('title', 'Do Homework')->first(); // Needs_approval = true
      ...
      $missedTaskDef = Task::where('title', 'Solo Math Challenge')->first(); // No recurrence
      ...
      $this->command->info('Skipping specific past status assignments as no children found.');
    }

    $this->command->info('Task and Assignment seeding completed.');
  }
}
